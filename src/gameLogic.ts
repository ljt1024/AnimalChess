...
export function initializeBoard(): CellData[] {
  ...
  // 4x4 board = 16 cells, exactly matching 16 pieces. No empty cells.
...
export function checkWinCondition(board: CellData[]): Faction | 'DRAW' | null {
  // If one side has no pieces left, the other wins.
  // Note: Only count REVEALED pieces? No, all pieces.
  // But wait, if pieces are hidden, you don't know if you won?
  // Usually, the game ends when one player has no pieces or cannot move.
  // For simplicity: eliminate all opponent pieces.
